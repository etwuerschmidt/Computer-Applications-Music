...
//set phase of the LFO
void Mu45LFO::setPhase(float phase)
{
	phase = phase * 512 / 180; // at 180 degrees, the wave will be halfway through its period
}

void Mu45LFO::setType(int type)
{
	if (type == 1) {
		float pi = 3.1415926535897932384626433832795;
		...
		// initialize a table with a sine waveform
		for (int i = 0; i < N; i++) {
			table[i] = (2 / pi)*asin(sin((2 * pi)*i/N));
		}
	}
}
