...
public:
    ...
    void paint (Graphics& g, LookAndFeel& lf)
    {
        if (style != IncDecButtons)
        {
            if (isRotary())
            {
                float sliderPos = (float) owner.valueToProportionOfLength (lastCurrentValue);
				
				//Added following line to avoid errors with 0/0
				sliderPos = (isnan(sliderPos) ? 0.0 : sliderPos);
